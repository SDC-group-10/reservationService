...
let generateBookedDates = () => { // currently limited to 100;
  let results = [];
  for (var i =0; i < 100; i++) {
    let obj = {};
...
let generateUsers = () => { //gets 10
  ...
  console.log(numbers.length, nouns.length, adjectives.length);
  for (var i = 0; i < /*adjectives.length*/27; i++) {
    for (var j = 0; j < /*nouns.length*/18; j++) {
      for (var k =0; k < numbers.length; k++) {
        let temp = [adjectives[i], nouns[k], numbers[j].join('')].join('');;
        results.push(temp);
      }
    }
  }
...
// let bookedDates = generateBookedDates();
// let numbers = getNumbers();
// console.log(numbers)
